...
const SideBar = ({ className }: { className?: string }) => {
  ...
  const menuItems: MenuItem[] = [
    ...
    },
    {
      ...
      onOff: pathname.includes('/course'),
    },
    {
      title: '이용안내',
    ...
    },
    {
      ...
      onOff: pathname.includes('/community'),
    },
  ];
  ...
  const changeCategory = (item: MenuItem) => {
    setCategory((prev) => prev.map((menu) => ({ ...menu, onOff: false })));
    setCategory((prev) =>
      prev.map((menu) =>
        menu.title === item.title ? { ...menu, onOff: true } : { ...menu, onOff: false }
      )
    );
  };
